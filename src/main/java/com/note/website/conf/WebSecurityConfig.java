...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		/*
		 * http.csrf().disable().authorizeRequests()//禁用CSRF保护 //任何访问都必须授权
		 * .anyRequest().fullyAuthenticated() .antMatchers("/login").permitAll()
		 * .antMatchers("/logout").permitAll() .antMatchers("/static/**").permitAll()
		 * //.antMatchers("/js/**").permitAll() //.antMatchers("/css/**").permitAll()
		 * //.antMatchers("/fonts/**").permitAll()
		 * //.antMatchers("/favicon.ico").permitAll() .antMatchers("/").permitAll()
		 * .anyRequest().authenticated()
		 * .sessionManagement().maximumSessions(1).sessionRegistry(sessionRegistry)
		 * .and().and().logout().invalidateHttpSession(true) .clearAuthentication(true)
		 * .and().httpBasic();
		 * httpBasic:参考http://blog.csdn.net/u012373815/article/details/56832167
		 */
		// 允许所有用户访问”/”和”/home”
		http.csrf().disable().authorizeRequests().antMatchers("/login", "/logout", "/register", "/error", "/static/**")
				.permitAll()
				// 其他地址的访问均需验证权限
				...
				// 退出登录后的默认网址是”/home”
				.logoutSuccessUrl("/login").permitAll().invalidateHttpSession(true).clearAuthentication(true).and()
				// 登录后记住用户，下次自动登录
